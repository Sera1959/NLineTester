public class MD5 {
  ...
  public void update(MD5State stat, byte buffer[], int offset, int length) {
    ...
    if((stat.count[0] += (length << 3)) < (length << 3)) stat.count[1]++;
    ...
    if(length >= partlen) {
      ...
      for(i = partlen; (i + 63) < length; i += 64) transform(stat, buffer, i);
      index = 0;
    } else i = 0;

    if(i < length) {
      start = i;
      for(; i < length; i++) stat.buffer[index + i - start] = buffer[i + offset];
    }
  }
  ...
  public synchronized byte[] finalState() {
    byte bits[];
    int index, padlen;
    MD5State fin;
  ...
  class MD5State {
    ...
    public MD5State(MD5State from) {
      ...
      for(i = 0; i < count.length; i++) this.count[i] = from.count[i];
    }
  }
